...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($nom && $photo && $nationality && $club && $flag && $logo && $position && $rating) {
    $query = "INSERT INTO players (name, photo_path, nationality, club, flag_path, logo_path, position, rating) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt && mysqli_stmt_execute($stmt)) {
      ...
      if ($position === 'GK') {
        ...
        if ($diving && $handling && $kicking && $reflexes && $speed && $positioning) {
          $query = "INSERT INTO goalkeeper_attributes 
                             (player_id, diving, handling, kicking, reflexes, speed, positioning) 
                             VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($shooting && $pace && $dribbling && $defending && $physical && $passing) {
          $query = "INSERT INTO outfield_attributes 
                             (player_id, shooting, pace, dribbling, defending, physical, passing) 
                             VALUES (?, ?, ?, ?, ?, ?, ?)";
...
<body>
  <div class="containerr" style="width: 100%">
    <div
      class="dashbord"
      style="width: 100%; background-color: #dbf1fc">
      <header
    ...
    <div class="fromData" style="display: none; width: 30%; margin: auto">
      <div
        ...
        <form action="http://localhost/projectBrifYoucode_CreationBackendGestionnaireContenuFUTChampions-Ultimate-Team/" method="post">
